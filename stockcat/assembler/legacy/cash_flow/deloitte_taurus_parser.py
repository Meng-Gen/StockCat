...
class DeloitteTaurusParser():
    ...
    def __parse_column_name_list(self):
        ...
        # sanity check 
        assert lines[3].strip() == u'合 併 現 金 流 量 表'

        column_name_list = [u'會計科目']
        for local_string in lines[6].split():
    ...
    def __scan_account_without_colon(self, local_string):
        ...
        if m:
            ...
            return self.__scan_account_with_parenthesis_dollar_number_dollar_number(local_string)

    def __scan_account_with_parenthesis_dollar_number_dollar_number(self, local_string):
        m = self.string_utils.match(u'^([^\s]*\(.*\)[^\s]*)\s*\$\s*([^\s]*)\s*\$\s*([^\s]*)$', local_string.strip())
        if m:
            return m
        else:
            return self.__scan_account_with_parenthesis_negative_number_negative_number(local_string)

    def __scan_account_with_parenthesis_negative_number_negative_number(self, local_string):
        m = self.string_utils.match(u'^([^\s]*\(.*\)[^\s]*)\s*(\(.*\))\s*(\(.*\))$', local_string.strip())
        if m:
            return m
        else:
            return self.__scan_account_with_parenthesis_positive_number_negative_number(local_string)

    def __scan_account_with_parenthesis_positive_number_negative_number(self, local_string):
        m = self.string_utils.match(u'^([^\s]*\(.*\)[^\s]*)\s*(.*)\s*(\(.*\))$', local_string.strip())
        if m:
            return m
        else:
            return self.__scan_account_with_parenthesis_negative_number_positive_number(local_string)

    def __scan_account_with_parenthesis_negative_number_positive_number(self, local_string):
        m = self.string_utils.match(u'^([^\s]*\(.*\)[^\s]*)\s*(\(.*\))\s*(.*)$', local_string.strip())
        if m:
            return m
        else:
            return self.__scan_account_with_parenthesis_positive_number_positive_number(local_string)

    def __scan_account_with_parenthesis_positive_number_positive_number(self, local_string):
        m = self.string_utils.match(u'^([^\s]*\(.*\)[^\s]*)\s*([^\s]*)\s*([^\s]*)$', local_string.strip())
        if m:
            return m
        else:
