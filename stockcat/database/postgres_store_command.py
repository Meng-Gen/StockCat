...
from stockcat.feed.cash_flow_feed import CashFlowFeed
...
class PostgresStoreCommand():
    ...
    def store_income_statement(self, feed):
        operation = 'INSERT INTO income_statement(release_date,stock_symbol,stmt_date,account,account_order,value) VALUES (%(release_date)s, %(stock_symbol)s, %(stmt_date)s, %(account)s, %(account_order)s, %(value)s)'
        self.__store_feed(operation, feed)

    def store_cash_flow(self, feed):
        operation = 'INSERT INTO cash_flow(release_date,stock_symbol,stmt_date,account,account_order,value) VALUES (%(release_date)s, %(stock_symbol)s, %(stmt_date)s, %(account)s, %(account_order)s, %(value)s)'
        self.__store_feed(operation, feed)
    """
