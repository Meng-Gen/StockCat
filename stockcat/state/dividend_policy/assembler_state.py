...
class AssemblerState(AriesState):
    ...
    def run(self):
        ...
        for entry in self.todo_entry_list:
            # update progress
            curr_count += 1
            self.logger.info('assemble dividend policy: {0} (progress: {1}/{2})'.format(entry, curr_count, entry_count))
            
            # assemble dividend policy
            content = self.spider.get_crawled(entry)
            self.dao[entry] = self.assembler.assemble(content, entry)
