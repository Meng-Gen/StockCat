...
class DatabaseState(AriesState):
    ...
    def run(self):
        ...
        for entry in self.todo_entry_list:
            # update progress
            curr_count += 1
            self.logger.info('store dividend policy: {0} (progress: {1}/{2})'.format(entry, curr_count, entry_count))

            # store dividend policy
            feed = self.feed_builder.build(value['dao'][entry])
