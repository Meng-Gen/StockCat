...
class SpiderState(AriesState):
    ...
    def run(self):
        ...
        for entry in self.todo_entry_list:
            # update progress
            curr_count += 1
            self.logger.info('crawl operating revenue: {0} (progress: {1}/{2})'.format(entry, curr_count, entry_count))

            # crawl stock_exchange_market operating revenue
            self.spider.crawl('stock_exchange_market', entry)
            self.avoid_blocking()

            # crawl otc_market operating revenue
            self.spider.crawl('otc_market', entry)
